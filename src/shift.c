...
void	twist_map_x_y_z(t_paint *paint, t_point *po, t_val *val)
{
	...
	while (po != NULL)
	{
		...
		po->z = -y0 * sin(paint->cos_sin_x) +
				po->z * cos(paint->cos_sin_x);
		po->x = val->max_x / 2 + x0 * cos(paint->cos_sin_y) +
				po->z * sin(paint->cos_sin_y);
		po->z = -x0 * sin(paint->cos_sin_y) + po->z * cos(paint->cos_sin_y);
		x0 = po->x - val->max_x / 2;
		y0 = po->y - val->max_y / 2;
		po->x = val->max_x / 2 + x0 * cos(paint->cos_sin_z) -
				y0 * sin(paint->cos_sin_z);
		po->y = val->max_x / 2 + x0 * sin(paint->cos_sin_z) +
				y0 * cos(paint->cos_sin_z);
		po = po->next;
	}
